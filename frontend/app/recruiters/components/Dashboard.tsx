...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const t = {
    ...
    cardBgHover: theme === "dark" ? "hover:bg-slate-800/50 hover:border-slate-700/50" : "hover:bg-gray-50 hover:border-gray-300",
    inputBg: theme === "dark" ? "bg-slate-800/50 border-slate-700/50" : "bg-gray-100 border-gray-300",
    ...
    // Charts
    chartGrid: theme === "dark" ? "#1e293b" : "#e5e7eb",
    chartAxis: theme === "dark" ? "#64748b" : "#6b7280",
    tooltipBg: theme === "dark" ? "#0f172a" : "#ffffff",
    tooltipBorder: theme === "dark" ? "#1e293b" : "#e5e7eb",
    tooltipText: theme === "dark" ? "#e2e8f0" : "#1f2937",
  };
    ...
    <ThemeContext.Provider value={theme}>
      <div className={`min-h-screen font-sans ${t.pageBg} transition-colors duration-200`}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={toggleTheme}
                  className={`p-2 rounded-lg ${t.btnSecondary} border transition-colors`}
                ...
                <button
                  ...
                  className={`hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${t.btnSecondary}`}
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="relative group">
                  <button className={`flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-lg border transition-colors ${theme === "dark" ? "bg-slate-800/50 border-slate-700/50 hover:bg-slate-800" : "bg-gray-100 border-gray-200 hover:bg-gray-200"}`}>
                    ...
                    <span className={`hidden sm:block text-sm ${t.textPrimary}`}>{recruiterName?.split(" ")[0] || "Recruiter"}</span>
                  ...
                  <div className={`absolute right-0 mt-2 w-48 py-1 rounded-lg border shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all ${t.dropdownBg}`}>
                    <div className={`px-4 py-2 border-b ${theme === "dark" ? "border-slate-800" : "border-gray-200"}`}>
                      <p className={`text-sm font-medium ${t.textPrimary}`}>{recruiterName}</p>
                      <p className={`text-xs ${t.textSecondary}`}>{recruiterEmail}</p>
                    </div>
                    <Link href="/recruiter-dashboard" className={`flex items-center gap-2 px-4 py-2 text-sm transition-colors ${theme === "dark" ? "text-slate-300 hover:text-white hover:bg-slate-800/50" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"}`}>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  ...
                  <p className={`mt-1 text-sm ${t.textSecondary}`}>Here's your hiring pipeline overview</p>
                </div>
                <div className="flex gap-2 sm:hidden">
                  ...
                  <button onClick={onOpenInvite} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border ${t.btnSecondary}`}>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard value={dynamicMetrics.totalApplicants} label="Total Applicants" color="blue" theme={theme} />
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className={`rounded-xl border p-5 transition-colors ${t.cardBg}`}>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className={`text-sm font-medium flex items-center gap-2 ${t.textPrimary}`}>
                        <svg className={`w-4 h-4 ${t.textSecondary}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      {!isEditingJobDesc && (
                        <button onClick={() => setIsEditingJobDesc(true)} className="text-xs text-blue-500 hover:text-blue-400 transition-colors">
                          Edit
                        </button>
                      )}
                    </div>
                    ...
                    {isEditingJobDesc ? (
                      <div className="space-y-3">
                        <textarea
                          ...
                          className={`w-full h-32 px-3 py-2 rounded-lg border text-sm resize-none focus:outline-none focus:border-blue-500/50 ${t.inputBg} ${t.textPrimary} ${t.textPlaceholder}`}
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => { setIsEditingJobDesc(false); setEditedJobDesc(jobDescription); }} className={`px-3 py-1.5 rounded-md text-xs transition-colors ${t.btnSecondary} border`}>
                            Cancel
                          </button>
                        </div>
                      </div>
                    ) : (
                      <p className={`text-sm leading-relaxed line-clamp-5 ${t.textSecondary}`}>
                  ...
                  {/* Quick Stats */}
                  <div className={`rounded-xl border p-5 transition-colors ${t.cardBg}`}>
                    <h3 className={`text-sm font-medium mb-4 flex items-center gap-2 ${t.textPrimary}`}>
                      <svg className={`w-4 h-4 ${t.textSecondary}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={stat.label} className={`flex items-center justify-between py-2 ${i < 3 ? `border-b ${theme === "dark" ? "border-slate-800/50" : "border-gray-200"}` : ''}`}>
                          <span className={`text-sm ${t.textSecondary}`}>{stat.label}</span>
                          <span className={`text-sm font-medium ${t.textPrimary}`}>{stat.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <ChartCard title="Skills Distribution" theme={theme}>
                      <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={[...skillsData]} barSize={20} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                          ...
                          <XAxis dataKey="name" stroke={t.chartAxis} fontSize={11} tickLine={false} axisLine={false} />
                          <YAxis stroke={t.chartAxis} fontSize={11} tickLine={false} axisLine={false} />
                          <Tooltip contentStyle={{ backgroundColor: t.tooltipBg, border: `1px solid ${t.tooltipBorder}`, borderRadius: "8px", fontSize: "12px" }} itemStyle={{ color: t.tooltipText }} />
                    ...
                    <ChartCard title="Score Trends" theme={theme}>
                      <ResponsiveContainer width="100%" height={180}>
                        <LineChart data={scoresTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke={t.chartGrid} />
                          <XAxis dataKey="month" stroke={t.chartAxis} fontSize={11} tickLine={false} axisLine={false} />
                          <YAxis stroke={t.chartAxis} fontSize={11} domain={[0, 10]} tickLine={false} axisLine={false} />
                          <Tooltip contentStyle={{ backgroundColor: t.tooltipBg, border: `1px solid ${t.tooltipBorder}`, borderRadius: "8px", fontSize: "12px" }} />
                          <Legend iconType="circle" iconSize={6} wrapperStyle={{ fontSize: 11, color: t.chartAxis }} />
                  ...
                  <ChartCard title="Experience Distribution" theme={theme}>
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        <Pie data={[...experienceData]} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name} (${(percent * 100).toFixed(0)}%)`} outerRadius={65} innerRadius={30} paddingAngle={2} dataKey="value">
                          {experienceData.map((_, index) => (
                            <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} stroke={theme === "dark" ? "#020617" : "#ffffff"} strokeWidth={2} />
                          ))}
                        </Pie>
                        <Tooltip formatter={(value, name) => [`${value} candidates`, name]} contentStyle={{ backgroundColor: t.tooltipBg, border: `1px solid ${t.tooltipBorder}`, borderRadius: "8px", fontSize: "12px" }} />
                      </PieChart>
                    </ResponsiveContainer>
                  </ChartCard>
                </div>
          ...
          {activeTab === "candidates" && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  <h2 className={`text-xl font-semibold ${t.textPrimary}`}>Candidates</h2>
                  <p className={`mt-1 text-sm ${t.textSecondary}`}>Review and manage applicants</p>
                </div>
                <div className={`text-sm ${t.textSecondary}`}>
                  Showing <span className={`font-medium ${t.textPrimary}`}>{filteredVideos.length}</span> of {videos.length}
                </div>
              </div>
              ...
              {filteredVideos.length > 0 ? (
                ...
                <div className={`text-center py-16 rounded-xl border ${t.cardBg}`}>
                  <div className={`w-14 h-14 mx-auto mb-4 rounded-xl flex items-center justify-center ${theme === "dark" ? "bg-slate-800/50" : "bg-gray-100"}`}>
                  ...
                  <p className={t.textPrimary}>No matching candidates</p>
                  <p className={`text-sm mt-1 ${t.textMuted}`}>Try adjusting your filters</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "questions" && recruiterId && (
            <div>
              <div className="mb-6">
                <h2 className={`text-xl font-semibold ${t.textPrimary}`}>Interview Questions</h2>
                <p className={`mt-1 text-sm ${t.textSecondary}`}>Manage questions for your interviews</p>
              </div>
              <InterviewQuestions recruiterId={recruiterId} theme={theme} />
            </div>
          )}
        </main>
        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
          {isChatOpen && (
            <div className={`w-80 rounded-xl border shadow-2xl overflow-hidden ${theme === "dark" ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200"}`}>
              <div className={`flex items-center justify-between px-4 py-3 border-b ${theme === "dark" ? "bg-slate-800/50 border-slate-800" : "bg-gray-50 border-gray-200"}`}>
                <span className={`text-sm font-medium ${t.textPrimary}`}>AI Assistant</span>
                <button onClick={() => setIsChatOpen(false)} className={`${t.textSecondary} hover:${t.textPrimary} transition-colors`}>
              ...
              <div className="p-4 space-y-3">
                <input
                  ...
                  className={`w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:border-blue-500/50 ${t.inputBg} ${t.textPrimary} ${t.textPlaceholder}`}
                ...
                {chatReply && (
                  <div className={`p-3 rounded-lg text-sm whitespace-pre-wrap max-h-40 overflow-auto ${theme === "dark" ? "bg-slate-800/50 text-slate-300" : "bg-gray-100 text-gray-700"}`}>
                    {chatReply}
                  </div>
                )}
              </div>
...
// Sub-components
const MetricCard = ({ value, label, color, theme }: { value: number; label: string; color: "blue" | "emerald" | "indigo" | "amber"; theme: Theme }) => {
  const colorMap = {
  ...
  return (
    <div className={`rounded-xl ${c.bg} border ${c.border} p-4 transition-colors`}>
      ...
      <div className={`text-sm mt-1 ${theme === "dark" ? "text-slate-400" : "text-gray-600"}`}>{label}</div>
    </div>
  );
};

const ChartCard = ({ title, children, theme }: { title: string; children: React.ReactNode; theme: Theme }) => (
  <div className={`rounded-xl border p-5 transition-colors ${theme === "dark" ? "bg-slate-900/50 border-slate-800/50" : "bg-white border-gray-200"}`}>
    <h3 className={`text-sm font-medium mb-4 ${theme === "dark" ? "text-white" : "text-gray-900"}`}>{title}</h3>
    {children}
  </div>
);

const CandidateCard = ({ video, isTopApplicant, onSelect, theme }: { video: Video; isTopApplicant: boolean; onSelect: () => void; theme: Theme }) => (
  <button
    onClick={onSelect}
  ...
  >
    {isTopApplicant && (
      <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-medium mb-3">
    ...
    <div className="flex items-start gap-3">
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm font-medium shrink-0 ${theme === "dark" ? "bg-slate-800 text-slate-300" : "bg-gray-100 text-gray-600"}`}>
        {video.candidate_details?.full_name?.charAt(0) || "?"}
      </div>
      <div className="flex-1 min-w-0">
        <p className={`font-medium text-sm truncate ${theme === "dark" ? "text-white" : "text-gray-900"}`}>{video.candidate_details?.full_name || video.title}</p>
        <p className={`text-xs truncate ${theme === "dark" ? "text-slate-500" : "text-gray-500"}`}>{video.candidate_details?.email || ""}</p>
      </div>
    </div>

    {video.candidate_details?.experience && (
      <div className={`mt-3 text-xs ${theme === "dark" ? "text-slate-500" : "text-gray-500"}`}>
        {video.candidate_details.experience} years experience
      </div>
    )}

    <div className="mt-3 flex items-center justify-between">
      <span className={`text-xs ${theme === "dark" ? "text-slate-600" : "text-gray-400"}`}>
        {video.created_at ? new Date(video.created_at).toLocaleDateString() : ""}
      </span>
      <span className="text-xs text-blue-500 group-hover:text-blue-400 transition-colors flex items-center gap-1">
        View
